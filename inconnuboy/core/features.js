...
const GiftedAntiLink = async (Gifted, message, getGroupMetadata) => {
    try {
        ...
        const { getGroupSetting, addAntilinkWarning, resetAntilinkWarnings } = require('../store/groupSettings');
        const { getSudoNumbers } = require('../store/sudo');
        const { getLidMapping } = require('../socket/cache');
...
const GiftedAntibad = async (Gifted, message, getGroupMetadata) => {
    try {
        ...
        const { getGroupSetting, addAntibadWarning, resetAntibadWarnings, getBadWords } = require('../store/groupSettings');
        const { getSudoNumbers } = require('../store/sudo');
        const { getLidMapping } = require('../socket/cache');
...
const GiftedAntiGroupMention = async (Gifted, message, getGroupMetadata) => {
    try {
        ...
        const { getGroupSetting, addAntiGroupMentionWarning, resetAntiGroupMentionWarnings } = require('../store/groupSettings');
        const { getSudoNumbers } = require('../store/sudo');
        const { getLidMapping } = require('../socket/cache');
...
async function getAIResponse(query) {
    if (isIdentityQuestion(query)) {
        return 'I am an Interactive Ai Assistant Chat Bot, created by INCONNU BOY!';
    }
...
const GiftedAntiDelete = async (Gifted, deletedMsg, key, deleter, sender, botOwnerJid, deleterPushName, senderPushName) => {
    ...
    const { getLidMapping, getGroupMetadata } = require('../socket/cache');
...
const GiftedAntiEdit = async (Gifted, updateData, findOriginal) => {
    try {
        ...
        const { getGroupMetadata } = require('../socket/cache');
